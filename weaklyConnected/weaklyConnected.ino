...
//byte mac[] = { 0x00, 0x10, 0x5C, 0xE0, 0x92, 0xFC};
byte mac[] = { 0xB4, 0x21, 0x8A, 0xF8, 0x6F, 0x53};
IPAddress ip(192, 168, 2,2);
...
//    timerKick.pulse(photograph, 3, LOW); // 10 minutes
    sensor1Num++;
...
//      timerKick.after(kickTime, timerIsr);
      ...
      //      digitalWrite(kick, LOW);
        MsTimer2::set(kickTime, timerIsr); // 500ms period
  MsTimer2::start();
    }
...
MsTimer2::set(kickTime * resultSensor1[index].imageInfoA.img_dataStatus, timerIsr); // 500ms period
  MsTimer2::start();
    }
    else
    {
